...
def import_from_parser(dict_cards, import_from):
    with open(import_from, 'r') as file:
        dict_cards = json.load(file)
    print(f'{len(dict_cards)} cards have been loaded.')
    return dict_cards


def export_to_parser(dict_cards, export_to):
    with open(args.export_to, 'w') as file:
        json.dump(dict_cards, file)
...
if args.import_from is not None:
    dict_cards = import_from_parser(dict_cards, arg.import_from)
...
if args.export_to is not None:
    export_to_parser(dict_cards, arg.export_to)
